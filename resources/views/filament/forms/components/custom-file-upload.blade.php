...
>
    <div class="flex items-center justify-center w-full">
        ...
        >
            <div class="flex flex-col items-center justify-center pt-4 pb-5 px-4 text-center">
                <div class="w-10 h-10 mb-2 rounded-full bg-white shadow-sm flex items-center justify-center group-hover:scale-110 transition-transform">
            ...
            <template x-if="isUploading">
                <div class="absolute inset-0 bg-white/80 flex flex-col items-center justify-center p-4">
                    <div class="w-full max-w-[150px] bg-gray-100 rounded-full h-1.5 overflow-hidden">
                        <div class="bg-[#ff3b5c] h-full transition-all duration-300" :style="'width: ' + progress + '%'"></div>
                    </div>
                    <p class="mt-2 text-[10px] font-bold text-[#ff3b5c]" x-text="'Uploading ' + progress + '%'"></p>
                </div>
            </template>
        </label>
    </div>
    ...
    <!-- Preview Grid -->
    <div 
        class="grid grid-cols-4 sm:grid-cols-5 gap-2 mt-3" 
        x-show="state && state.length > 0"
    >
        <!-- Persistent Previews (from state) -->
        <template x-for="(file, index) in state" :key="index">
            <div class="relative group aspect-square rounded-lg overflow-hidden border border-gray-100 bg-gray-50 shadow-sm">
                <img 
                    :src="file.startsWith('livewire-file:') ? previews[index] : '/storage/' + file" 
